...
class FilterItemsAdapter(var itemList: ArrayList<CategoryOptionsData>) :
    ...
    override fun onBindViewHolder(holder: FilterItemsAdapter.ViewHolder, position: Int) {
        ...
        if (itemList[position].form_type == 1) {
            ...
            holder.min.addTextChangedListener(
                    object : TextWatcher {
                        ...
                        override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                            context.getSharedPreferences("filterOptions", 0)
                                    .edit()
                                    .putString(
                                            holder.id.text.toString(),
                                            holder.min.text.toString() +
                                                    "-" +
                                                    holder.max.text.toString()
                                    )
                                    .apply()
                        }
            ...
            holder.max.addTextChangedListener(
                    object : TextWatcher {
                        override fun beforeTextChanged(
                                p0: CharSequence?,
                                p1: Int,
            ...
            holder.enter.addTextChangedListener(
                    object : TextWatcher {
                        override fun beforeTextChanged(
                                p0: CharSequence?,
                                p1: Int,
                        ...
                        override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                            context.getSharedPreferences("filterOptions", 0)
                                    .edit()
                                    .putString(
                                            itemList[holder.adapterPosition].id.toString(),
                                            holder.enter.text.toString()
                                    )
                                    .apply()
                        }
                        override fun afterTextChanged(p0: Editable?) {}
        ...
        holder.dropList.setOnClickListener { showDropListDialog(holder) }
        holder.value.setOnClickListener { showDropListDialog(holder) }
    }

    private fun showDropListDialog(holder: ViewHolder) {
        val data = ArrayList<com.application.adverial.ui.model.DropList>()
        val values =
                itemList[holder.adapterPosition].values?.split(",")?.map { it.trim() }
                        ?: emptyList()

        // For checkbox type, add a single option
        if (itemList[holder.adapterPosition].type == "checkbox") {
            data.add(com.application.adverial.ui.model.DropList("Yes", "1"))
            data.add(com.application.adverial.ui.model.DropList("No", "0"))
        } else {
            // For select type, add all values
            for (value in values) {
                data.add(com.application.adverial.ui.model.DropList(value, value))
            }
        }
        ...
        dialog.getStatus().observe(context as LifecycleOwner) {
            ...
            context.getSharedPreferences("filterOptions", 0)
                    .edit()
                    .putString(itemList[holder.adapterPosition].id.toString(), it.id)
                    .apply()
        }
    }
    ...
    fun getResult(): MutableLiveData<kotlin.String> {
        return result
    }
}
