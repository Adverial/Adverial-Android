...
import kotlinx.android.synthetic.main.activity_phone_auth.lottie15
...
class VerifyWa : AppCompatActivity() {
    ...
    fun verify(view: View) {
        ...
        if (otp != null) {
            ...
            repo.verifyOtpWa(whatsappNumber, otp).observe(this) {
    ...
    fun resendCode(view: View) {
        lottie15.visibility = View.VISIBLE
        val repo = AuthRepository(this)
        repo.resendOtpWa(whatsappNumber).observe(this) {
            lottie15.visibility = View.GONE
        ...
        code.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                when {
                    ...
                    code.text.length == 1 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 1)
                    }
                    code.text.length == 2 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 2)
                    }
                    code.text.length == 3 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 3)
                    }
                    code.text.length == 4 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 4)
                    }
                    code.text.length == 5 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 5)
                    }
                    code.text.length == 6 -> {
//                        updateCodeBackground(c1, c2, c3, c4, c5, c6, 6)
                    }
                }
            }
    ...
    private fun resetCodeBackground(vararg textViews: TextView) {
        textViews.forEachIndexed { index, textView ->
            textView.background = ContextCompat.getDrawable(this, if (index == 0) R.drawable.stroke2 else R.drawable.stroke1)
            textView.text = ""
            textView.isCursorVisible = index == 0
        }
    }

    private fun updateCodeBackground(vararg textViews: Int, length: Int) {
        textViews.forEachIndexed { index, textView ->
//            textView.background = ContextCompat.getDrawable(this, if (index == length) R.drawable.stroke2 else R.drawable.stroke1)
//            textView.isCursorVisible = index == length
//            if (index < length) textView.text = verify_otp_code.text.substring(index, index + 1)
        }
    }
