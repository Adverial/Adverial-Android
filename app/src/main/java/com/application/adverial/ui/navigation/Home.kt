...
class Home : AppCompatActivity() {
            ...
            runOnUiThread {
                repo.getMainCategoryData().observe(this@Home) {
                    if (it.status) {
                        ...
                        menuCategoriesRecyclerView.adapter = MenuCategoryAdapter(list as List<SubCategory>)
    ...
    fun language(view: View){
        ...
        dialog.getStatus().observe(this) {
            getSharedPreferences("user", 0).edit().putString("languageName", it.name).putString("languageId", it.id).putInt("languageFlag", it.flag).apply()
            updateLanguageUI()
        }
    }
    ...
    fun menu(view: View){ home_drawerLayout.openDrawer(Gravity.START) }
    ...
    fun voice(view: View){
        ...
        }catch (e: Exception) { Toast.makeText(this, getString(R.string.voiceError), Toast.LENGTH_SHORT).show() }
    }
