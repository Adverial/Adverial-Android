...
import com.application.adverial.ui.dialog.AdFeedback
import com.application.adverial.ui.dialog.AlertDialog
import com.application.adverial.ui.dialog.AlertDialog1

class MyAdsAdapter(var itemList: ArrayList<Ad>) : RecyclerView.Adapter<MyAdsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyAdsAdapter.ViewHolder, position: Int) {
        ...
        if(itemList[position].city_detail != null && itemList[position].country_detail != null){
            ...
            holder.date.text= itemList[position].created_at!!.split("T")[0]
        }
        holder.remove.setOnClickListener {
            val dialog= AlertDialog1(context.resources.getString(R.string.attention), context.resources.getString(R.string.removeMyAd))
            dialog.show((context as AppCompatActivity).supportFragmentManager, "AlertDialog")
            dialog.getStatus().observe(context as LifecycleOwner, { answer ->
                if(answer == "ok"){
                    val dialog1= AdFeedback(itemList[holder.adapterPosition].id.toString())
                    dialog1.show((context as AppCompatActivity).supportFragmentManager, "AdFeedback")
                    dialog1.getStatus().observe(context as LifecycleOwner, { itt->
                        if(itt == "finish"){
                            val repo= Repository(context)
                            repo.removeAd(itemList[holder.adapterPosition].id.toString())
                            repo.getRemoveAdData().observe(context as LifecycleOwner, {
                                if(it.status){
